        ...
        <div className="relative container mx-auto px-4">
          ...
          <div className="grid lg:grid-cols-3 gap-12">
              ...
              >
                {event.categories.map((categoryId, index) => {
                  ...
                  return (
                    <span
                      key={index}
                      className={`px-4 py-2 rounded-full text-sm font-medium bg-glass-medium backdrop-blur-xs ${categoryInfo?.color || 'text-white'}`}
                    >
            ...
            <div className="lg:col-span-1">
              ...
              >
                <div className="bg-glass-light backdrop-blur-xs rounded-2xl p-8 border border-white/10 mb-6">
                  {/* Price */}
                    ...
                    <div className="text-4xl font-bold text-white mb-2">
                      {event.price.type === 'free'
                        ...
                        : event.price.earlyBird
                          ? `${event.price.earlyBird} ${event.price.currency}`
                  ...
                  <motion.button
                    onClick={handleShare}
                    className="w-full px-6 py-3 bg-glass-medium backdrop-blur-xs border border-white/20 text-white font-medium rounded-xl hover:bg-glass-light transition-all duration-300 flex items-center justify-center"
                    whileHover={{ scale: 1.02 }}
                    whileTap={{ scale: 0.98 }}
                  >
                ...
                {/* Event Info */}
                <div className="bg-glass-light backdrop-blur-xs rounded-2xl p-6 border border-white/10">
                  <h3 className="text-lg font-semibold text-white mb-4">Event Info</h3>
                  <div className="space-y-3 text-sm text-white/70">
                    <div>
                      <strong className="text-white">Registreringsdeadline:</strong>
                      <br />
            ...
            >
              ...
              <div className="space-y-4">
                {event.agenda.map((item, index) => (
                  <div
                    key={index}
                    className="bg-glass-light backdrop-blur-xs rounded-xl p-6 border border-white/10"
                  >
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-4">
                      <div className="flex items-center space-x-4 mb-2 md:mb-0">
                        <div className="text-accent-blue font-mono font-semibold">{item.time}</div>
                        <h3 className="text-lg font-semibold text-white">{item.title}</h3>
                      </div>
                      {item.speaker && (
                        <div className="text-sm text-white/70">Speaker: {item.speaker}</div>
                      )}
                    </div>
                  ...
                  >
                    <div className="flex items-start space-x-4">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-white/70 text-sm leading-relaxed">{speaker.bio}</p>
                      </div>
                    </div>
                  </div>
                ))}
                  ...
                  <Link href={`/events/${relatedEvent.slug}`}>
                    ...
                    <div className="p-6">
                      ...
                      <div className="flex items-center justify-between text-sm text-white/60">
                        ...
                        <span>
                          {relatedEvent.price.type === 'free'
                            ? 'Gratis'
                            : `${relatedEvent.price.amount} DKK`}
                        </span>
                      </div>
                    </div>
                  </Link>
      ...
      {showRegistrationModal && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <motion.div
            ...
            className="bg-glass-light backdrop-blur-xs rounded-2xl border border-white/10 p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto"
          >
            ...
            <form onSubmit={handleRegistration} className="space-y-6">
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setRegistrationData({ ...registrationData, company: e.target.value })
                    }
                    className="w-full px-4 py-3 bg-glass-medium border border-white/20 rounded-xl text-white placeholder-white/50 focus:outline-none focus:border-accent-blue"
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setRegistrationData({ ...registrationData, experience: e.target.value })
                  }
              ...
              <div>
                <label className="block text-white mb-2">
                  Hvad håber du at få ud af dette event?
                </label>
                <textarea
                  rows={4}
                  value={registrationData.expectations}
                  onChange={(e) =>
                    setRegistrationData({ ...registrationData, expectations: e.target.value })
                  }
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                ...
                <motion.button
                  type="button"
                  onClick={() => setShowRegistrationModal(false)}
                  className="flex-1 px-6 py-4 bg-glass-medium backdrop-blur-xs border border-white/20 text-white font-semibold rounded-xl hover:bg-glass-light transition-all duration-300"
                  whileHover={{ scale: 1.02 }}
                  whileTap={{ scale: 0.98 }}
                >
                  Annuller
